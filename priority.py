...
def priority():
    print("NON PRE-EMPTIVE PRIORITY SCHEDULLING")
    ...
    for i in range(n):
        key = "P"+str(i+1)
        c = int(input("Enter prioity of process"+str(i+1)+": "))
    ...
    dn=d.values()
    #print(dn)
    d = sorted(d.items(), key=lambda item: item[1][2],reverse=True)
    #print(d)
    # print(d)
    k=n
    comp=0
    ...
    for i in range(n):
        print("   ",d[i][0],"   |   ",d[i][1][2],"   |   ",d[i][1][0]," |    ",d[i][1][1]," |    ",CT[i],"    |    ",TAT[i],"  |   ",WT[i],"   |  ")
